...
# Redefine the function to support vector multipliers
simulate_likert7 <- function(n, mean_score = 4, sd_score = 1.5, 
                             multiplier = 1, 
                             higher_is_more = TRUE) {
  
  # 1. Generate the base scores
  latent <- rnorm(n, mean = mean_score, sd = sd_score)
  
  # 2. Calculate the shift (vectorized)
  ...
  # 4. Round and clamp to 1-7 range
...
df_sim <- df_sim %>%
  mutate(
    edu_modifier = case_when(
      # Wrap the vectors in list() to create a list-column
      A2_age %in% c("18-29", "30-39") ~ list(c(0.25, 0.15, 0.18, 0.20, 0.22)),
      A2_age %in% c("40-49", "50-59") ~ list(c(0.38, 0.18, 0.15, 0.14, 0.15)),
      TRUE ~ list(c(0.55, 0.18, 0.12, 0.08, 0.07))  # 60+
    )
  ) %>%
  rowwise() %>% # Now acts on the list-column created above
  mutate(
    A4_education = sample(
...
df_sim <- df_sim %>%
  ...
  mutate(
    urban_probs = list({
      # 1. Calculate the remaining probability available for non-dense categories
      p_remaining <- 1 - prop_urban_dense
      
      # 2. Define the base weights for the other 3 categories from your urban_props
      # (Intermédiaire: 0.29, Rural Péri: 0.19, Rural Isolé: 0.14)
      other_weights <- c(0.29, 0.19, 0.14)
      
      # 3. Scale these weights so they sum up to p_remaining
      other_probs <- p_remaining * (other_weights / sum(other_weights))
      
      # 4. Combine: [Dense Prob, Scaled Intermédiaire, Scaled Péri, Scaled Isolé]
      c(prop_urban_dense, other_probs)
    }),
    urbanization = sample(
      urban_props$urbanization,
      1,
      # Probabilities are already normalized by logic, but sum() ensures safety
...
df_sim <- df_sim %>%
  mutate(
    j2_base_prob = case_when(
      # Wrap the vectors in list() so they fit in a single cell
      geo_zone == "South" ~ list(c(0.03, 0.08, 0.82, 0.07)),
      geo_zone == "Central" ~ list(c(0.01, 0.04, 0.88, 0.07)),
      TRUE ~ list(c(0.005, 0.02, 0.92, 0.055))
    )
  ) %>%
  rowwise() %>%
  mutate(
...
df_sim <- df_sim %>%
  mutate(
    j11_probs = case_when(
      # Wrap the vectors in list()
      geo_zone == "South" ~ list(c(0.15, 0.10, 0.08, 0.12, 0.25, 0.30)),
      geo_zone == "Central" ~ list(c(0.05, 0.08, 0.03, 0.07, 0.35, 0.42)),
      TRUE ~ list(c(0.02, 0.05, 0.01, 0.04, 0.40, 0.48))
    )
  ) %>%
  rowwise() %>%
  mutate(
...
# J12: Awareness of fumigation operations (Fixing this proactively)
df_sim <- df_sim %>%
  mutate(
    j12_probs = case_when(
      # Wrap the vectors in list()
      geo_zone == "South" ~ list(c(0.12, 0.15, 0.45, 0.28)),
      geo_zone == "Central" ~ list(c(0.05, 0.08, 0.55, 0.32)),
      TRUE ~ list(c(0.02, 0.04, 0.60, 0.34))
    )
  ) %>%
  rowwise() %>%
  mutate(
